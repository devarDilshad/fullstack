...
export default function Home({ refreshInvoice, setProduct }: any) {
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            // refresh order
            refreshInvoice(formData.invoice_id);
            await getProduct();
            // reset forData state
            setFormData({
                invoice_id: '',
    ...
    // get aa specific product with validation and error handling
    const getProduct = async () => {
        try {
            const response = await axios.get(`${url}/products/${formData.product_id}`)
    ...
    return (
        <div className="mx-auto my-10 p-6 bg-white rounded-lg shadow-md w-full">
            <h2 className="text-2xl font-bold mb-4">Create an order for an invoice</h2>
            <form className="flex flex-col md:flex-row justify-center items-center gap-6" onSubmit={handleSubmit}>
                ...
                <div className="mb-4">
                    ...
                    <input
                        ...
                        className="mt-1 ps-2 py-0.5 block w-full rounded-sm border-gray-300 shadow-sm focus:ring-2 focus:outline-0 focus:ring-slate-400 focus:ring-opacity-50"
                    />
                </div>

                {/* Type */}
                <div className="mb-4">
                    <label className="block text-gray-700">Container Type</label>
                    <input
                        required
                        type="text"
                        name="containerType"
                ...
                {/* Qty */}
                <div className="mb-4">
                    <label className="block text-gray-700">Qunatity</label>
                    <input
                        required
                        type="number"
                        name="quantity"
                ...
                <button
                    type="submit"
                    className="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 min-w-28"
                >
                    Add Order
                </button>
            </form>
